...
func PostSymbol(s iexcloud.IEXSymbol) error {
	defer wg.Done()
	...
	input := &ddb.PutItemInput{
		Item:      av,
		TableName: aws.String("IEXSymbol"),
	}
...
func GetPutRequests(symbols []iexcloud.IEXSymbol) ([]*ddb.PutRequest, error) {
...
func Handler(ctx context.Context) (*Response, error) {
	...
	putRequests, err := GetPutRequests(symbols)
	if err != nil {
		return apiResponse(http.StatusBadRequest, ErrorBody{ErrorMsg: aws.String(err.Error())})
	}
	// make the batchwriterequests
	batchWriteRequests := GetBatchWriteRequests(putRequests, "IEXSymbol")
	// post to ddb
	...
	body, err := json.Marshal(map[string]interface{}{
		"success": fmt.Sprintf("Successfully updated %d records in %s\n", len(putRequests), "IEXSymbol"),
	})
	if err != nil {
		return apiResponse(http.StatusBadRequest, ErrorBody{ErrorMsg: aws.String(err.Error())})
	}
